...
const expressStaticGzip = __webpack_require__(16);
const getFiles = __webpack_require__(17);
const serverReact = __webpack_require__(19);
const serverNarratives = __webpack_require__(33);
...
if (devServer) {
  webpack = __webpack_require__(12); // eslint-disable-line
  config = __webpack_require__(35); // eslint-disable-line
  webpackDevMiddleware = __webpack_require__(36); // eslint-disable-line
  webpackHotMiddleware = __webpack_require__(37); // eslint-disable-line
}
...
app.use(__webpack_require__(38)({
  reverse: true
}));
...
var fetch = __webpack_require__(18); // not needed for local data
var request = __webpack_require__(3);
// const prettyjson = require('prettyjson');

// const validUsers = ['guest', 'mumps', 'lassa'];

var getDataFile = function getDataFile(res, filePath, s3) {
...
var getStaticFile = function getStaticFile(res, filePath) {
...
var getManifest = function getManifest(query, res) {
...
var getSplashImage = function getSplashImage(query, res) {
  getDataFile(res, query.src, query.s3);
};

var getImage = function getImage(query, res) {
  getStaticFile(res, query.src);
};

var getDatasetJson = function getDatasetJson(query, res) {
...
var ReactMarkdown = __webpack_require__(11); /* https://github.com/rexxars/react-markdown */
var fs = __webpack_require__(2);
var path = __webpack_require__(0);
var request = __webpack_require__(3);

var serveStaticPost = function serveStaticPost(query, res) {
  ...
  if (global.LOCAL_STATIC) {
    ...
    var md = fs.readFileSync(path.join(global.LOCAL_STATIC_PATH, query.path), 'utf8');
    res.send(ReactDOMServer.renderToStaticMarkup(React.createElement(ReactMarkdown, { source: md })));
  } else {
    request(global.REMOTE_STATIC_BASEURL + query.path, function (err, response, body) {
      if (err || body.startsWith("404") || body.split("\n")[1].startsWith('<head><title>404')) {
        res.status(404).send('Post not found.');
        return;
      }
      res.send(ReactDOMServer.renderToStaticMarkup(React.createElement(ReactMarkdown, { source: body })));
    });
  }
};

module.exports = {
  serveStaticPost: serveStaticPost
};
...
var p=__webpack_require__(6),r=__webpack_require__(1),t=__webpack_require__(4),w=__webpack_require__(7),x=__webpack_require__(8),y=__webpack_require__(9),aa=__webpack_require__(10);
...
var makeBlock = function makeBlock() {
  ...
  return { lines: [], url: url };
};

var parsePreamble = function parsePreamble(blocks, lines) {
  if (!lines[0].startsWith("---")) {
    return 0;
  }
  var block = makeBlock(); /* initialise */
  var contents = { title: false, author: false, date: false, category: false };
  var n = 1;
...
var parseMarkdownArray = function parseMarkdownArray(mdArr) {
  var blocks = [];
  var nMax = mdArr.length;
  var reUrl = /url=[^\s]+\?([^\s`]*)/;
  var n = parsePreamble(blocks, mdArr); /* modifies blocks in-place */
  var block = makeBlock(); /* initialise */
  while (n < nMax) {
    var line = mdArr[n];
    if (line.startsWith('`nextstrain') && line.match(reUrl)) {
      if (block.lines.length && block.lines.filter(function (l) {
        return l.length;
      }).length) {
...
var convertBlocksToHTML = function convertBlocksToHTML(blocks) {
  ...
  try {
    for (var _iterator = blocks[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      block.__html = ReactDOMServer.renderToStaticMarkup(React.createElement(ReactMarkdown, { source: block.lines.join("\n") }));
      delete block.lines;
    }
  } catch (err) {
...
var serveNarrative = function serveNarrative(query, res) {
  if (global.LOCAL_STATIC) {
    /* this code is syncronous, but that's ok since this is never used in production */
    var mdArr = fs.readFileSync(path.join(global.LOCAL_STATIC_PATH, "narratives", query.name + ".md"), 'utf8').split("\n");
    var blocks = parseMarkdownArray(mdArr);
    convertBlocksToHTML(blocks);
    res.send(JSON.stringify(blocks).replace(/</g, '\\u003c'));
  } else {
    request(global.REMOTE_STATIC_BASEURL + "narratives/" + query.name + ".md", function (err, response, body) {
      if (err || body.startsWith("404") || body.split("\n")[1].startsWith('<head><title>404')) {
        res.status(404).send('Post not found.');
        return;
      }
      var mdArr = body.split("\n");
      var blocks = parseMarkdownArray(mdArr);
      convertBlocksToHTML(blocks);
      res.send(JSON.stringify(blocks).replace(/</g, '\\u003c'));
    });
...
module.exports = require("express-naked-redirect");
